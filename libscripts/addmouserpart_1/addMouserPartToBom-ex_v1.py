...
# eg python addMouserPartToBom-ex.py ATMEGA644A-AU-ND
# Mouser add part
import sys
...
with open(PARTSDB, 'a') as file:
	...
	# check to see if mouser part number is the first argument
	...
	if len(sys.argv)<2:
		...
		# cjeck that Mouser Part Number exists 
		# if fails display error message
		except urllib2.HTTPError as e:
		    print("The server couldn\'t fulfill the request."+"\nMouser Part Number "+partNo+" is not available on website")
		    # tkMessageBox.showerror("Error","The server couldn\'t fulfill the request"+"\Mouser Part Number "+partNo+" is not available on website")
 
		except urllib2.URLError as e:
		    print('We failed to reach a server.')
		    print('Reason: ', e.reason) 
		else:
			# Mouser Part Number exists go for it
			...
			for elem in soup(text=re.compile(r'Mfr. No')):
			...
			if mfgPartNo is not None and mfg is not None and desc is not None and package is not None:
				...
				# tkMessageBox.showinfo("Information","Mouser Part Number: "+partNo+" has been added to parts db")
